...
resource "azurerm_data_factory_linked_service_azure_databricks" "adb-link" {
  name                = "LS-ADB-ViaMSI"
  data_factory_id     = azurerm_data_factory.adf_ws.id
  ...
  access_token = databricks_token.pat.token_value
   depends_on = [ azurerm_databricks_workspace.adb_workspace, databricks_cluster.shared_autoscaling, databricks_token.pat ]
}
